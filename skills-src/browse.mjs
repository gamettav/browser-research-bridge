// Canonical definition for the `browse` research command.
//
// This is the SINGLE SOURCE OF TRUTH. Both the Claude Code and Codex skills,
// the Claude plugin command, and the Codex agents/openai.yaml metadata are
// generated from this file by `scripts/generate-skills.mjs`. Never hand-edit the
// generated SKILL.md / command / yaml files — edit here and re-run the generator
...
export const allowedToolIds = TOOLS.map((tool) => `mcp__${MCP_SERVER}__${tool}`);

// Drives BOTH the slash-command name and natural-language auto-invocation.
// Written to fire on research intent and stay quiet on everything else.
export const DESCRIPTION =
  "Research the web to answer a question, look something up, verify a claim, find sources, " +
  "check the latest on a topic, or read and summarize one or more URLs — through the local " +
  "Browser Research bridge, which renders pages in your own Chrome when ordinary fetching is " +
  "blocked, incomplete, or needs JavaScript. Use when the user asks to research, look this up, " +
  "browse the web, search for sources, find out about, check online, or read a link. Do NOT use " +
  "for coding, local files, math, or conversation that needs no web sources.";

export const ARGUMENT_HINT = "[quick:|deep:] <question or URL(s)>";

export const CATEGORY = "Productivity";

export const DEFAULT_PROMPT =
  "Research this topic through Browser Research and answer with cited final source URLs.";

// Harness-specific invocation tokens substituted into BODY.
export const TOKENS = {
  claude: { invoke: "/browse", debug: "/browse debug last", tool: "the read-only research tools" },
  codex: { invoke: "$browse", debug: "$browse debug last", tool: "the read-only research tools" }
};

// The shared workflow body. `{{INVOKE}}` and `{{DEBUG}}` are replaced per harness.
export const BODY = `# Browse — web research

Answer the user's question by researching the public web through the local Browser
Research bridge. Invoke explicitly as \`{{INVOKE}} <question>\`, or activate automatically
...
If the user runs \`{{DEBUG}}\`, reveal the technical diagnostics for the most recent research
