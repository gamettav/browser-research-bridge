...
GroundTab is a read-only local research bridge. Install the extension only in a Chrome or Brave profile whose site access you are comfortable granting to the agent. Browser site-access controls can narrow the default all-public-sites permission; a separate low-privilege profile remains the strongest option for users who research while signed in to sensitive services, but it is not part of ordinary installation.
...
- Pairing binds the broker to the exact GroundTab extension origin. Other extension origins and web origins are rejected.
